...
import { useSearchBox } from '@/composables/search'
...
.DocSearch-Button-Keys {
  @apply justify-between min-w-[12rem];
}
.DocSearch-Button-Key {
  @apply static w-5 h-5 pb-0 border border-[color:var(--docsearch-muted-color)] m-0 top-0;
}
...
.DocSearch-Button:hover .DocSearch-Button-Key {
  @apply text-[color:var(--docsearch-text-color)] border-[color:var(--docsearch-text-color)];
}
.DocSearch-Form,
.DocSearch-Hit a {
  @apply rounded-lg;
...
.DocSearch-Button:hover .DocSearch-Search-Icon {
  @apply text-[color:var(--docsearch-text-color)];
}
.DocSearch-Input {
  padding-left: 1rem;
  font-size: 1em;
